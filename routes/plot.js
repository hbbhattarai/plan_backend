...
router.get('/get-plot/:plot_id', (req, res) => {
    const plotId =(req.params.plot_id).toUpperCase();
    var table = "plots";
    pool.query(`SELECT * FROM ${table} WHERE plot_id='${plotId}' `, (err, results) => {
        res.setHeader("Access-Control-Allow-Origin", "*")
        res.json(results.rows[0]);
    })
})

router.get('/plots/:z/:x/:y.pbf', (req, res) => {
    var table = "plots";
