...
router.get('/precincts/:z/:x/:y.pbf', (req, res) => {
    var table = "precincts";
...
router.get('/get-precinct/:planid', (req, res) => {
    var table = "precincts";
    var planid = req.params.planid;
    pool.query(`SELECT * FROM ${table} where plan_id=${planid}`, (err, results) => {
        if (err) {
            throw err
        } else {
            const precient = results.rows;
            res.status(200).json(precient);
